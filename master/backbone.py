...
from attention import RGA_Module


WEIGHT_PATH = os.path.join(os.path.dirname(__file__), '../')+'/weights/pre_train/resnet50-19c8e357.pth'
#WEIGHT_PATH = "C:\\Users\mingx\PycharmProjects\DL_RGA_ReID_Project\weights"
...
class RGA_ResNet50_alpha(nn.Module):
    def __init__(self, pretrained=True, last_stride=1, block=Bottleneck, layers=[3, 4, 6, 3],
                 s_ratio=8, c_ratio=8, d_ratio=8, height=256, width=128, model_path=WEIGHT_PATH):
        super(RGA_ResNet50_alpha, self).__init__()

        self.in_channels = 64

        # RestNet-50 transform
        ...
        # RGA Modules
        self.rga_att1 = RGA_Module(256, (height // 4) * (width // 4),
                                   cha_ratio=c_ratio, spa_ratio=s_ratio, down_ratio=d_ratio)
        self.rga_att2 = RGA_Module(512, (height // 8) * (width // 8),
                                   cha_ratio=c_ratio, spa_ratio=s_ratio, down_ratio=d_ratio)
        self.rga_att3 = RGA_Module(1024, (height // 16) * (width // 16),
                                   cha_ratio=c_ratio, spa_ratio=s_ratio, down_ratio=d_ratio)
        self.rga_att4 = RGA_Module(2048, (height // 16) * (width // 16),
                                   cha_ratio=c_ratio, spa_ratio=s_ratio, down_ratio=d_ratio)
